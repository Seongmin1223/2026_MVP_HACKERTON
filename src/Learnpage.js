...
export default function LearnPage({ onHome, onStudy }) {
  ...
  const onMouseDown  = useCallback((e) => {
    if (e.target.tagName === "TEXTAREA") return; // textarea 드래그 방지
          ...
          <div className="inner">
            ...
            <div className="learn-top-bar">
              <div className="learn-tabs">
                {tabs.map(t => (
                  <button key={t} className={`learn-tab${activeTab===t?" active":""}`} onClick={()=>setActiveTab(t)}>{t}</button>
                ))}
              </div>
