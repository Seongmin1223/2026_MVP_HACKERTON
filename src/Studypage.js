  ...
  <svg viewBox="0 0 58 58" fill="none">
    <defs>
      <linearGradient id="gearG1" x1="0%" y1="0%" x2="100%" y2="100%">
        <stop offset="0%" stopColor="#7ecae6"/><stop offset="100%" stopColor="#3a9bbf"/>
      </linearGradient>
    ...
    {[0,45,90,135,180,225,270,315].map((a,i) => {
      ...
      return <rect key={i} x={x-2.2} y={y-2.2} width="4.4" height="4.4" rx="1" fill="url(#gearG1)" transform={`rotate(${a},${x},${y})`} />;
    })}
    {/* 작은 기어 */}
    ...
    {[0,60,120,180,240,300].map((a,i) => {
      ...
      return <rect key={i} x={x-1.8} y={y-1.8} width="3.6" height="3.6" rx="0.8" fill="url(#gearG2)" transform={`rotate(${a},${x},${y})`} />;
    })}
    {/* 눈 — 큰 기어 위에 작은 밝은 원 */}
...
const ChipIcon = () => (
  <svg viewBox="0 0 58 58" fill="none">
    ...
    {/* 핀 — 위 */}
    {[14,22,30,38].map((x,i) => <rect key={`t${i}`} x={x} y="8" width="3" height="8" rx="1.5" fill="#60a5fa" />)}
    {/* 핀 — 아래 */}
    {[14,22,30,38].map((x,i) => <rect key={`b${i}`} x={x} y="42" width="3" height="8" rx="1.5" fill="#60a5fa" />)}
    {/* 핀 — 왼쪽 */}
    {[18,26,34].map((y,i) => <rect key={`l${i}`} x="8" y={y} width="8" height="3" rx="1.5" fill="#60a5fa" />)}
    {/* 핀 — 오른쪽 */}
    {[18,26,34].map((y,i) => <rect key={`r${i}`} x="42" y={y} width="8" height="3" rx="1.5" fill="#60a5fa" />)}
    {/* 본체 */}
  ...
  <svg viewBox="0 0 58 58" fill="none">
    <defs>
      <linearGradient id="rocketBody" x1="0%" y1="100%" x2="100%" y2="0%">
        <stop offset="0%" stopColor="#f59e0b"/><stop offset="50%" stopColor="#fcd34d"/><stop offset="100%" stopColor="#fff"/>
      </linearGradient>
...
const AtomIcon = () => (
  <svg viewBox="0 0 58 58" fill="none">
    ...
    {/* 궤도 타원 1 */}
    <ellipse cx="29" cy="29" rx="22" ry="10" fill="none" stroke="rgba(167,139,250,0.45)" strokeWidth="1.5" transform="rotate(-30,29,29)" />
    {/* 궤도 타원 2 */}
    <ellipse cx="29" cy="29" rx="22" ry="10" fill="none" stroke="rgba(103,232,249,0.45)" strokeWidth="1.5" transform="rotate(30,29,29)" />
    {/* 궤도 타원 3 */}
    <ellipse cx="29" cy="29" rx="22" ry="10" fill="none" stroke="rgba(134,239,172,0.45)" strokeWidth="1.5" transform="rotate(90,29,29)" />
    {/* 궤도 위 전자 구간 */}
  ...
  <svg viewBox="0 0 58 58" fill="none">
    <defs>
      ...
      <linearGradient id="beakerG" x1="0%" y1="0%" x2="100%" y2="100%">
        <stop offset="0%" stopColor="rgba(100,200,180,0.3)"/><stop offset="100%" stopColor="rgba(60,140,130,0.2)"/>
      </linearGradient>
    </defs>
    {/* 비커 본체 */}
    <path d="M18,16 L18,44 Q18,48 22,48 L36,48 Q40,48 40,44 L40,16" fill="url(#beakerG)" stroke="rgba(52,211,153,0.5)" strokeWidth="1.2" strokeLinejoin="round" />
    {/* 입구 가장자리 */}
    ...
    {/* 액체 */}
    <path d="M18,30 Q22,28 29,32 Q35,28 40,30 L40,44 Q40,48 36,48 L22,48 Q18,48 18,44 Z" fill="url(#liquid)" opacity="0.8" />
    {/* 거품 */}
...
const SUBJECTS = [
  {
    ...
    glowColor: "#7ecae6",
    title: "기계 공학",
    desc: "3D 기계 구조와 동작 원리를 체험하세요.",
  },
  {
    icon: <ChipIcon />,
    glowColor: "#60a5fa",
    title: "전기 전자 공학",
    desc: "3D 회로와 반도체 구조를 학습하세요.",
  },
  {
    icon: <RocketIcon />,
    glowColor: "#fcd34d",
    title: "항공 우주 공학",
    desc: "3D 우주 기술과 항공 구조를 탐구하세요.",
  },
  {
    icon: <AtomIcon />,
    glowColor: "#a78bfa",
    title: "재료 과학",
    desc: "3D 원자 구조와 소재 특성을 분석하세요.",
  },
  {
    icon: <BeakerIcon />,
    glowColor: "#34d399",
    title: "화학 공학",
    desc: "3D 화학 공정과 반응을 시뮬레이션하세요.",
  },
  {
    icon: <BridgeIcon />,
    glowColor: "#60a5fa",
    title: "토목 공학",
    desc: "3D 구조 설계와 건축 원리를 탐색하세요.",
  },
];
...
/*  StudyPage 컴포넌트                           */
/* ════════════════════════════════════════════ */
export default function StudyPage({ onHome, onLearn }) {
        ...
        <section className="study-body">
          <div className="inner">
            ...
            <div className="study-grid">
              {SUBJECTS.map((s, i) => (
                <div key={i} className="study-card" onClick={onLearn} style={{cursor:"pointer"}}>
